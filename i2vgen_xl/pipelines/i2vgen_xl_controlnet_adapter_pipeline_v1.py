...
class I2VGenXLControlNetAdapterPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        n_ref_frames = len(image) if isinstance(image, list) else 1
        

        # 0. Default height and width to unet
        orig_height = height or self.unet.config.sample_size * self.vae_scale_factor
        orig_width = width or self.unet.config.sample_size * self.vae_scale_factor
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if (control_model_input_h, control_model_input_w) != (64, 64):
